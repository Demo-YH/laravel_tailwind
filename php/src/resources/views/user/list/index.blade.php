<x-user-app>
    ...
    @if (session('saveDraft'))
            <x-message :message="session('saveDraft')" type="saveDraft" />
        @endif
        @if (session('release'))
            <x-message :message="session('release')" type="release" />
        @endif
        @if (session('reservationRelease'))
            <x-message :message="session('reservationRelease')" type="reservationRelease" />
        @endif
        @if (session('status'))
            <x-message :message="session('status')" type="status" />
        @endif
        @if (session('error'))
        ...
        @if (session('updateSaveDraft'))
            <x-message :message="session('updateSaveDraft')" type="updateSaveDraft" />
        @endif
        @if (session('updateRelease'))
            <x-message :message="session('updateRelease')" type="updateRelease" />
        @endif
        @if (session('updateReservationRelease'))
            <x-message :message="session('updateReservationRelease')" type="updateReservationRelease" />
        @endif
        @if (session('moveTrash'))
            <x-message :message="session('moveTrash')" type="moveTrash" />
        @endif
        <div class="h-screen overflow-y-scroll">
            <div class="px-4 sm:px-4">
                <div class="flex justify-between">
                    ...
                    <div class="pt-4">
                        <a href="{{ route('post.create', ['id' => Auth::id()]) }}">
                                    ...
                                    <tr>
                                        ...
                                        <td class="px-5 py-5 mr-5 border-b border-gray-200 bg-white text-sm">
                                            <div class="flex">
                                                ...
                                                @csrf
                                                    <button type="submit" onClick="return confirm('ゴミ箱に移動しますか？');" class="underline text-red-700 whitespace-no-wrap">
                                                        Trash
                                                    </button>
                                                </form>
                                            </div>
