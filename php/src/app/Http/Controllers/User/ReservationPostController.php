...
class ReservationPostController extends Controller
{
     ...
     * __construct
     */
    public function __construct(protected Post $post, protected Category $category, ReservationPost $reservationPost)
    {
        $this->post = $post;
    ...
    public function reservationStore(Request $request)
    {
        ...
        $post = $this->post->insertPostToArticle($user_id, $request, $publish_flg);
        ...
        // 予約公開設定内容をreservation_postsテーブルにinse rt
     ...
     * @param $post_id 投稿ID
     */
    public function reservationEdit(Request $request, int $post_id)
    {
        // ログインユーザー情報を取得
        $user_id = auth()->user()->id;
     ...
     * @param $post_id 投稿ID
     */
    public function reservationUpdate(Request $request, int $post_id)
    {
        // ログインユーザー情報を取得
        $user_id = auth()->user()->id;
